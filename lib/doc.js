...
export function setFieldValue(pdf, fields, fieldName, value) {
  ...
  if (field) {
    if (currentField.type === "TextField") {
      ...
      // Choice fields: pdf-lib reports single-select as "Dropdown" and
      // multi-select as "OptionList"; ComboBox/ListBox/RadioGroup are kept
      // as aliases. All are set via select().
      currentField.type === "Dropdown" ||
      currentField.type === "OptionList" ||
      currentField.type === "ComboBox" ||
      currentField.type === "ListBox" ||
      currentField.type === "RadioGroup"
    ) {
      field.select(value);
    } else if (currentField.type === "Button") {
      field.setValue(value);
    } else {
      console.error(`Field ${fieldName} type ${currentField.type} not supported`);
    }
  } else {
